...
{
    public static class GetDoors
    {
        public static async Task<List<to.Door>> GetDoorsList(int projectId)
        {
            List<to.Door> doors = new List<to.Door>();
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    var response = await client.GetAsync("https://ucsh.protechtitan.com/titan/api/customer/projects/" + projectId + "/doors"); // _url =api endpoint url
                    if (response != null)
                    {
                        ...
                        try
                        {
                            var result = JsonConvert.DeserializeObject<to.Doors>(jsonString);
                            doors = ((to.Doors)result).Data.ToList();
                        }
