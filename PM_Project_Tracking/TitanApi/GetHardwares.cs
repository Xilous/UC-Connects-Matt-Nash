...
{
    public static class GetHardwares
    {
        public static async Task<List<to.Hardware>> GetHardwaresList(int projectId)
        {
            List<to.Hardware> hardwares = new List<to.Hardware>();
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    var response = await client.GetAsync("https://ucsh.protechtitan.com/titan/api/customer/projects/" + projectId + "/hardware"); // _url =api endpoint url
                    if (response != null)
                    {
                        ...
                        try
                        {
                            var result = JsonConvert.DeserializeObject<to.Hardwares>(jsonString);
                            hardwares = ((to.Hardwares)result).Data.ToList();
                        }
