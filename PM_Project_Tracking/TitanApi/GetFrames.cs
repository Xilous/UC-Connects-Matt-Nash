...
namespace PM_Project_Tracking.TitanApi
{
    ...
    {
        public static async Task<List<to.Frame>> GetFramesList(int projectId)
        {
            List<to.Frame> frames = new List<to.Frame>();
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    var response = await client.GetAsync("https://ucsh.protechtitan.com/titan/api/customer/projects/" + projectId + "/frames"); // _url =api endpoint url
                    if (response != null)
                    {
                        ...
                        try
                        {
                            var result = JsonConvert.DeserializeObject<to.Frames>(jsonString);
                            frames = ((to.Frames)result).Data.ToList();
                        }
